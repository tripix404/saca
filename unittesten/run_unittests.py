...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "ddos_attack"))
)
...
def run_specific(test_name):
    ...
    if not os.path.exists(test_path):
        print(
            f"❌ Testbestand '{test_name}' niet gevonden in map '{os.path.basename(SCRIPT_DIR)}'."
        )
        return
    ...
    if keuze == "1":
        ...
        for fname in sorted(os.listdir(SCRIPT_DIR)):
            if fname.startswith("unittest_") and fname.endswith(".py"):
                print(f" - {fname}")
        test_name = input(
            "Voer de naam van het unittest-bestand in (bijv. unittest_arp_scanner.py): "
        ).strip()
