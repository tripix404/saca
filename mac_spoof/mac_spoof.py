...
def generate_valid_mac() -> str:
    ...
    return ":".join(
        [f"{first_byte:02X}"] + [f"{random.randint(0x00, 0xFF):02X}" for _ in range(5)]
    )
                    ...
                    if subkey_name.isdigit():
                        with winreg.OpenKey(root_key, subkey_name) as subkey:
                            try:
                                ...
                                component_id = winreg.QueryValueEx(
                                    subkey, "ComponentId"
                                )[0].lower()
                                guid = winreg.QueryValueEx(subkey, "NetCfgInstanceId")[
                                    0
                                ]

                                if any(
                                    s in component_id
                                    for s in ["tap", "virtual", "vmxnet", "vpn", "wan"]
                                ):
                                    continue

                                try:
                                    mac = winreg.QueryValueEx(subkey, "NetworkAddress")[
                                        0
                                    ]
...
def verify_mac_change(guid: str, expected_mac: str) -> bool:
    ...
    try:
        key_path = rf"SYSTEM\CurrentControlSet\Control\Class\{{4D36E972-E325-11CE-BFC1-08002BE10318}}\{guid}"
        with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, key_path) as key:
            current_mac = winreg.QueryValueEx(key, "NetworkAddress")[0]
            return (
                current_mac.replace(":", "").upper()
                == expected_mac.replace(":", "").upper()
            )
    except:
        return False
...
def change_mac(interface_key: str, new_mac: str, guid: str) -> bool:
    ...
    try:
        ...
        key_path = rf"SYSTEM\CurrentControlSet\Control\Class\{{4D36E972-E325-11CE-BFC1-08002BE10318}}\{interface_key}"
        with winreg.OpenKey(
            winreg.HKEY_LOCAL_MACHINE, key_path, 0, winreg.KEY_WRITE
        ) as key:
            winreg.SetValueEx(
                key, "NetworkAddress", 0, winreg.REG_SZ, new_mac.replace(":", "")
            )
        ...
        for cmd in ps_commands:
            ...
            if "No MSFT_NetAdapter" in result:
                run_command(
                    f'netsh interface set interface "{guid}" admin=disable', shell=True
                )
                time.sleep(1)
                run_command(
                    f'netsh interface set interface "{guid}" admin=enable', shell=True
                )
                break
